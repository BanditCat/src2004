...
NVEB_EFFECT_NAME("Simple Render To Texture");
NVEB_EFFECT_VERSION("1.0");
NVEB_EFFECT_LOCATION("Effects\\Basic");
NVEB_EFFECT_ABOUT(0, NULL, "SimpleRenderTexture", "NVEffectsExplained.htm#SimpleRenderTexture");
NVEB_EFFECT_ABOUT(1, "Date", "March 2002", NULL);
...
void init()
{
    ...
    look_at(camera_mat, vec3_null, vec3_neg_z, vec3_y);
...
void reset()
{
    ...
    for ( int i = 0; i < 2; ++i)
    {
        manip[i]->reset();
        if (i == 0)
            manip[i]->set_eye_pos(vec3(nv_zero, nv_zero, 2.0f));
        else
            manip[i]->set_eye_pos(vec3(nv_zero, nv_zero, .5f));
