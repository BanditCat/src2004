...
RGBSurface::RGBSurface( const char* fn ) : dirty( true ), data( NULL ), 
					     wdth( 0 ), hght( 0 ){
  glGenTextures( 1, &glnm );
  SDL_Surface* srf = IMG_Load( fn );
  if( !srf ){
    char em[ 200 ];
    char* beg = "Failed to load texture bitmap ";
    char* mid = " with this error: ";
    char* sdle = SDL_GetError();
    if( ( strlen( beg ) + strlen( fn ) + strlen( mid ) + strlen( sdle ) ) > 199 )
      strcpy( em, "Failed to load texture" );
    else{
      strcpy( em, beg );
      strcat( em, fn );
      strcat( em, mid );
  ...
  wdth = srf->w;
  hght = srf->h;
  size_t e = srf->w * srf->h * 3;
  data = new byte[ e ];
  for( size_t i = 0; i < e; i++ )
    data[ i ] = ( (byte*)( srf->pixels ) )[ i ];
...
void RGBSurface::setpixel( const Point& c, const Pixel& p ){
  dirty = true;
  byte* ofs = data + ( c.x + ( ( hght - c.y ) - 1 ) * wdth ) * 3;
  *ofs++ = p.r;
  *ofs++ = p.g;
  *ofs++ = p.b;
}
...
SDL_Surface* RGBSurface::fliprgbsurf( SDL_Surface* srfc ){
  SDL_Surface* fsrf = SDL_CreateRGBSurface( 0, srfc->w, srfc->h, 24, 255,
					    (dword)255 << 8,
					    (dword)255 << 16,
					    0 );
#ifdef DBG
  cout << "Flipping texture" << endl;
#endif
  if( !fsrf ){
    char em[ 200 ];
  ...
  for( int y = 0; y < srfc->h; y++ ){
    memcpy( (byte*)fsrf->pixels + y * srfc->w * 3, 
	    (byte*)srfc->pixels + ( srfc->h - ( y + 1 ) ) * srfc->w * 3,
	    srfc->w * 3 );
  }
