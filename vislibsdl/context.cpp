...
Display::Display( word w, word h, const char* nm ) : light( 0, 0, 0 ),
  ...
  glptrActiveTexture = (PFNGLACTIVETEXTUREARBPROC)SDL_GL_GetProcAddress( "glActiveTextureARB" );
  if( glptrActiveTexture == NULL )
    throw vis_environment_error( "Failed to get proccess address for glActiveTexture" );

  glptrMultiTexCoord3f = (PFNGLMULTITEXCOORD3FARBPROC)SDL_GL_GetProcAddress( "glMultiTexCoord3fARB" );
  if( glptrMultiTexCoord3f == NULL )
    throw vis_environment_error( "Failed to get proccess address for glMultiTexCoord3f" );
  glptrMultiTexCoord2f = (PFNGLMULTITEXCOORD2FARBPROC)SDL_GL_GetProcAddress( "glMultiTexCoord2fARB" );
  if( glptrMultiTexCoord2f == NULL )
    throw vis_environment_error( "Failed to get proccess address for glMultiTexCoord2f" );
...
void Display::loadnormalmap( void ){
  RGBSurface srf( 256, 256 );

#ifdef DBG
  cout << "Initializing normal map" << endl;
#endif
  ...
  for( size_t i = 0; i < 256; i++ ){
    for( size_t j = 0; j < 256; j++ ){
      ...
      u.normal();
      ( (byte*)( srf.data ) )[ ( i + j * 256 ) * 3 ] = (byte)( ( u.x + 1 ) * 127.5f );  
      ( (byte*)( srf.data ) )[ ( i + j * 256 ) * 3 + 1 ] = (byte)( ( u.y + 1 ) * 127.5f );  
      ( (byte*)( srf.data ) )[ ( i + j * 256 ) * 3 + 2 ] = (byte)( ( u.z + 1 ) * 127.5f );  
    }
  }
  glTexImage2D( GL_TEXTURE_CUBE_MAP_POSITIVE_Z, 0, GL_RGB, srf.width(), srf.height(), 
		0, GL_RGB, GL_UNSIGNED_BYTE, srf.data );	
  for( size_t i = 0; i < 256; i++ ){
    for( size_t j = 0; j < 256; j++ ){
  ...
  glTexImage2D( GL_TEXTURE_CUBE_MAP_POSITIVE_Y, 0, GL_RGB, srf.width(), srf.height(), 
		0, GL_RGB, GL_UNSIGNED_BYTE, srf.data );	
  for( size_t i = 0; i < 256; i++ ){
    for( size_t j = 0; j < 256; j++ ){
  ...
  glTexImage2D( GL_TEXTURE_CUBE_MAP_NEGATIVE_Y, 0, GL_RGB, srf.width(), srf.height(), 
		0, GL_RGB, GL_UNSIGNED_BYTE, srf.data );	
  for( size_t i = 0; i < 256; i++ ){
    for( size_t j = 0; j < 256; j++ ){
  ...
  glTexImage2D( GL_TEXTURE_CUBE_MAP_POSITIVE_X, 0, GL_RGB, srf.width(), srf.height(), 
		0, GL_RGB, GL_UNSIGNED_BYTE, srf.data );	
  for( size_t i = 0; i < 256; i++ ){
    for( size_t j = 0; j < 256; j++ ){
  ...
  glTexImage2D( GL_TEXTURE_CUBE_MAP_NEGATIVE_X, 0, GL_RGB, srf.width(), srf.height(), 
		0, GL_RGB, GL_UNSIGNED_BYTE, srf.data );	
  for( size_t i = 0; i < 256; i++ ){
    for( size_t j = 0; j < 256; j++ ){
  ...
  glTexImage2D( GL_TEXTURE_CUBE_MAP_NEGATIVE_Z, 0, GL_RGB, srf.width(), srf.height(), 
		0, GL_RGB, GL_UNSIGNED_BYTE, srf.data );	

  GLenum err = glGetError();
  if( err != GL_NO_ERROR )
    throw vis_environment_error( "OpenGL error while loading the normal map" );
